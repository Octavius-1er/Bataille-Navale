          ...
          <div className="card glow" style={{ padding:28, position:'relative' }}>
            ...
            <div className="field" style={{marginBottom:0}}>
              ...
              <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginTop:4 }}>
                {(subject === 'Latin' ? [
                  { key:'positive',      label:'🔵 Singulier', color:'#00d4ff' },
                  { key:'negative',      label:'🟣 Pluriel',   color:'#aa44ff' },
                ] : [
              ...
              <div style={{ fontFamily:'Share Tech Mono,monospace',fontSize:9,color:'#4a7090',marginTop:6 }}>
                {subject === 'Latin'
                  ? 'L\'élève donne la forme au singulier et/ou au pluriel.'
      ...
      {step === 3 && (
        <div className="fade-up">
          ...
          <div style={{ display:'flex', gap:10, marginBottom:16, flexWrap:'wrap', alignItems:'center' }}>
            {subject === 'Latin' ? <>
              {forms.includes('positive')      && <span style={{ fontFamily:'Share Tech Mono,monospace',fontSize:10,color:'#00d4ff',border:'1px solid rgba(0,212,255,.3)',padding:'2px 8px' }}>🔵 Singulier activé</span>}
              {forms.includes('negative')      && <span style={{ fontFamily:'Share Tech Mono,monospace',fontSize:10,color:'#aa44ff',border:'1px solid rgba(170,68,255,.3)',padding:'2px 8px' }}>🟣 Pluriel activé</span>}
            </> : <>
                        ...
                        <td key={ci} style={{ padding:5, border:'1px solid #0d1e30', verticalAlign:'top' }}>
                          <div style={{ background:'#080f1c', padding:8 }}>
                            ...
                            {forms.includes('positive') && <>
                              <div style={{ fontFamily:'Share Tech Mono,monospace',fontSize:9,color:subject==='Latin'?'#00d4ff':'#00ff88',marginBottom:2 }}>{subject==='Latin'?'🔵 Singulier':'➕ Positif'}</div>
                              <input value={positive} onChange={e=>setCell(ri,ci,{...cell,positive:e.target.value})}
                                placeholder={subject==='Latin'?'Ex: rosa':'Ex: I ate a brownie.'}
                                style={{ ...inp({padding:'5px 8px',fontSize:11,marginBottom:4}) }}/>
                            </>}
                            {/* Negative / Pluriel */}
                            {forms.includes('negative') && <>
                              <div style={{ fontFamily:'Share Tech Mono,monospace',fontSize:9,color:subject==='Latin'?'#aa44ff':'#ff6666',marginBottom:2 }}>{subject==='Latin'?'🟣 Pluriel':'➖ Négatif'}</div>
                              <input value={negative} onChange={e=>setCell(ri,ci,{...cell,negative:e.target.value})}
                                placeholder={subject==='Latin'?'Ex: rosae':'Ex: I didn\'t eat a brownie.'}
                                style={{ ...inp({padding:'5px 8px',fontSize:11,marginBottom:4}) }}/>
                            </>}
                            {/* Interrogative — not shown for Latin */}
                            {forms.includes('interrogative') && subject !== 'Latin' && <>
